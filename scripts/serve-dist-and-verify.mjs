...
const distDir = path.resolve(process.env.VERIFY_DIST_DIR || 'dist')
const host = process.env.VERIFY_DIST_HOST || '127.0.0.1'
const requestedPort = Number(process.env.VERIFY_DIST_PORT || 0)
...
function assertDistExists() {
  if (!fs.existsSync(path.join(distDir, 'index.html'))) {
    console.error(`${distDir}: missing index.html; run a build before verify:dist`)
    process.exit(1)
  }
}
...
function runVerifier(baseUrl) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, ['scripts/verify-static-migration.mjs'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        VERIFY_BASE_URL: baseUrl,
        VERIFY_REQUIRE_STRICT_404: process.env.VERIFY_REQUIRE_STRICT_404 || '1',
      },
      stdio: 'inherit',
    })
...
server.listen(requestedPort, host, async () => {
  ...
  const code = await runVerifier(baseUrl)
