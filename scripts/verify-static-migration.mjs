...
const redirects = [
  ...
  ['/telegram-map/', '/karta-postov-telegram/'],
  ['/posts/', '/ru/blog/'],
...
async function verifyImportedArticle({ path, title, lang = 'ru', updatedAt, publishedAt }) {
  ...
  const expectedModifiedDate = updatedAt || publishedAt
  assert(expectedModifiedDate, `${path}: imported article is missing updatedAt/publishedAt`)
  assert(html.includes(`"dateModified": "${expectedModifiedDate}T00:00:00+03:00"`), `${path}: JSON-LD dateModified mismatch`)
...
async function verifyCrawlerFiles() {
  ...
  assert(!/https:\/\/okhlopkov\.com\/[^\s)]+\.md/.test(llms), '/llms.txt: links to robots-disallowed markdown sidecar URLs')
  assert(llms.includes(`${siteUrl}/ru/blog/`), '/llms.txt: missing RU blog canonical link')
  assert(llms.includes(`${siteUrl}/ru/articles/`), '/llms.txt: missing RU articles canonical link')
  assert(llms.includes(`${siteUrl}/ru/blog/ai-agents-s-chego-nachat/`), '/llms.txt: missing generated blog canonical link')
  assert(llms.includes(`${siteUrl}/claude-code-setup-mcp-hooks-skills-2026/`), '/llms.txt: missing Claude Code setup canonical link')
  assert(llms.includes(`${siteUrl}/privacy/`), '/llms.txt: missing privacy canonical link')
