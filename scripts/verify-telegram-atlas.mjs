...
assert.match(appSource, /function LegacyTelegramMapRedirect\(\)/, 'The old route must use a query-preserving redirect component')
assert.match(appSource, /pathname:\s*['"]\/karta-postov-telegram\/['"][\s\S]+search:\s*location\.search[\s\S]+hash:\s*location\.hash/, 'The SPA redirect must preserve query parameters and hashes')
...
assert.match(prerenderSource, /window\.location\.search/, 'Static redirects must preserve query parameters')
assert.match(prerenderSource, /window\.location\.hash/, 'Static redirects must preserve hashes')
assert.match(prerenderSource, /items:\s*\[TELEGRAM_MAP_FALLBACK_ITEM,\s*\.\.\.generatedRuBlog\]/, 'Russian blog fallback must list the Telegram map first')
assert.match(prerenderSource, /## Свежие материалы[\s\S]+\[Карта моих постов в Telegram\]\(\/karta-postov-telegram\/\)/, 'Homepage fallback must link to the Telegram map first')
